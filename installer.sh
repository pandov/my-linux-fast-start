...
sudo apt-fast -y install nvidia-384 nvidia-settings
sudo nvidia-xconfig

echo "\n\n[INSTALL TOUCHPAD GESTURES]\n\n"
sudo apt-fast -y install libinput-tools xdotool
...
sudo apt-fast update
sudo apt-fast -y install plank

echo "\n\n[INSTALL DARK THEME & TWEAK TOOL]\n\n"
sudo apt-fast install unity-tweak-tool
sudo add-apt-repository ppa:noobslab/themes
sudo apt-fast update
sudo apt-fast -y install arc-theme

echo "\n\n[INSTALL UTILS]\n\n"
sudo apt-fast -y install preload indicator-cpufreq dconf-editor virtualbox ubuntu-make wget
...
sudo apt-fast update
sudo apt-fast -y install oracle-java8-installer oracle-java8-set-default
sudo apt-fast -y install libc6:i386 libncurses5:i386 libstdc++6:i386 lib32z1 libbz2-1.0:i386
...
sudo apt-fast -y install software-properties-common apt-transport-https
#sudo apt-fast update
